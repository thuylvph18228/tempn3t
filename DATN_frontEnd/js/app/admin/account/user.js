function user($scope, $http) {
  ...
  getAllUser();
    ...
    $scope.searchUser = () => {
      $scope.page.page = 0;
      if ($scope.userName) {
        getAllUser(apiUser + "/get-by-username?username=" + $scope.userName);
      } else {
        $scope.userName = "";
        getAllUser(apiUser);
      }
    ...
    //         $scope.pageIndex  = 0;
    //         getAllUser(0, $scope.status);
    //     } else {
    //         $scope.pageIndex--;
    //         getAllUser($scope.pageIndex, $scope.status);
    //     }
    // }
    ...
    //         getAllUser($scope.totalPage - 1, $scope.status);
    //     } else {
    //         $scope.pageIndex++;
    //         getAllUser($scope.pageIndex, $scope.status);
    //     }
    ...
    $scope.updateStatus = (userID) => {
      $scope.isLoading = true;
      $http.put(apiUser + "/" + userID)
        .then(res => {
          $scope.isLoading = false;
        })
        .catch(err => {
    ...
    // check status
    $scope.updateStatusCustomer = (employee, index) => {
